// backend/scripts/import_suppliers.js
// Usage: node import_suppliers.js [concurrency=20]
// Env: TARGET=https://... (optional)

const fs = require('fs');
const path = require('path');

const csvPath = path.join(__dirname, 'suppliers_100.csv');
const concurrency = parseInt(process.argv[2] || '20', 10);
const TARGET = process.env.TARGET || 'https://pos-backend.posai.workers.dev/api/suppliers';
...
async function main(){
  ...
  console.log(`\n🚀 Loading ${items.length} suppliers from ${csvPath}`);
  ...
  async function worker() {
    while (true) {
      ...
      const payload = {
        name: r.name || `Supplier ${i+1}`,
        email: r.email || undefined,
        phone_num: r.phone_num || undefined,
        address: r.address || undefined,
        created_user: r.created_user || 'admin',
        total_stock: r.total_stock ? parseInt(r.total_stock, 10) : 0,
      };
  ...
  console.log(`\n✨ Done: posted ${success}/${items.length} suppliers`);
  if (success === items.length) {
    console.log('🎉 All suppliers imported successfully!');
  } else {
    console.log(`⚠️  ${items.length - success} suppliers failed`);
  }
}
